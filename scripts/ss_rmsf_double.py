...
with open(rmsf_xvg_path_2, 'r') as file:
    atoms_now = 0
    for line in file:
        # Check when the atoms start
        if atoms_now == 1:
            rmsf_values_2.append(float(line.split()[1]))
        elif line.startswith("@TYPE xy"):
            atoms_now = 1
...
# Plot RMSF values
ax.plot(final_data[0], rmsf_values, label="RMSF for C-alpha (Run 1)", color="black", linewidth=1)

ax.plot(final_data[0], rmsf_values_2, label="RMSF for C-alpha (Run 2)", color="darkgreen", linewidth=1)
...
ax.legend()
plt.title("TrmD Monomer Cα RMSF")
